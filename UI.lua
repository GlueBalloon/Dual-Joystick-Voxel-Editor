...
-- Base class under all others
UI.Panel = class()

function UI.Panel:init(x,y,w,h)
    --[[
    if tostring(stroke()) ~= "255" then
        print("Panel init: ", stroke())
    else
        print("Panel detected basic white")
    end
    ]]
    ...
    --[[
    if tostring(stroke()) ~= "255" then
        print("Panel end init: ", stroke())
    else
        print("Panel end init detected basic white")
    end
    ]]
end
...
function UI.Panel:draw() 
    ...
    if self.stroke then 
        stroke(self.stroke or color(248, 35, 22))
...
function UI.Button:init(x,y,w,h,t,a)
    ...
    --I have no idea why stroke() on its own doesn't work
    --It took so long to hunt this down I won't even question it:
...
function UI.Label:init(x,y,w,h,t,a)
    --[[
    if tostring(stroke()) ~= "255" then
        print("Label init before panel: ", stroke())
    else
        print("Label detected basic white")
    end
    ]]
    UI.Panel.init(self, x,y,w,h)
    self.text = t
...
--[[
    if tostring(stroke()) ~= "255" then
        print("Label end init: ", stroke())
    else
        print("Label end init detected basic white")
    end
]]
end
